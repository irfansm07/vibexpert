...
const rewardsData = {
  dailyTasks: [
    ...
    { id: 'like_10', title: 'Spread Love', desc: 'Like 10 posts', reward: 5, icon: '❤', completed: false },
  ...
  exclusiveRewards: [
    ...
    { id: 'frame', title: 'Golden Frame', desc: 'Profile border', cost: 300, icon: '🖼', category: 'cosmetic' },
...
const musicLibrary = [
  ...
  { id: 4, name: "Summer Vibes", artist: "Tropical", duration: "3:30", url: "https://assets.mixkit.co/music/preview/mixkit-summer-vibes-129.mp3", emoji: "🏖" },
...
async function verifyResetCode(e) {
  ...
  if(!code || code.length !== 6) return showMessage('⚠ Enter 6-digit code', 'error');
  if(!newPassword || !confirmPassword) return showMessage('⚠ Enter password', 'error');
  if(newPassword !== confirmPassword) return showMessage('⚠ Passwords don\'t match', 'error');
  if(newPassword.length < 6) return showMessage('⚠ Min 6 characters', 'error');
...
async function createPost() {
  ...
  if (!postText && selectedFiles.length === 0 && !selectedMusic && selectedStickers.length === 0) {
    return showMessage('⚠ Add content', 'error');
  }
  ...
  if (selectedPostDestination === 'community') {
    if (!currentUser.communityJoined || !currentUser.college) {
      showMessage('⚠ Join university first', 'error');
      setTimeout(() => {
  ...
  try {
    ...
    if (error.message.includes('timeout')) {
      showMessage('⚠ Timeout - try smaller images', 'error');
    ...
    html += `
      <div class="enhanced-post" id="post-${post.id}">
        <div class="enhanced-post-header">
          ...
          ${isOwn ? <button class="post-delete-btn" onclick="deletePost('${post.id}')">🗑</button> : ''}
        </div>
        ...
        <div class="enhanced-post-footer">
          ...
          <div class="enhanced-post-engagement">
            <button class="engagement-btn ${isLiked ? 'liked' : ''}" id="like-btn-${post.id}" onclick="toggleLike('${post.id}')">${isLiked ? '❤ Liked' : '❤ Like'}</button>
...
async function toggleLike(postId) {
  if (!currentUser) return showMessage('⚠ Login to like', 'error');
  try {
...
function openCommentModal(postId) {
  if (!currentUser) return showMessage('⚠ Login to comment', 'error');
      ...
      html += `
        <div class="comment-item" style="background:rgba(15,25,45,0.9);border:1px solid rgba(79,116,163,0.2);border-radius:12px;padding:15px;margin-bottom:10px;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
            ...
            ${isOwn ? <button onclick="deleteComment('${comment.id}','${postId}')" style="background:rgba(255,107,107,0.2);color:#ff6b6b;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:12px;">🗑</button> : ''}
          </div>
...
async function submitComment(postId) {
  ...
  if (!content) return showMessage('⚠ Empty comment', 'error');
  try {
  ...
  switch(platform) {
    ...
    case 'native':
      if (navigator.share) {
        ...
        showMessage('⚠ Not supported', 'error');
      }
      break;
  }
  try {
  ...
  musicLibrary.forEach(music => {
    ...
    musicItem.innerHTML = `
      ...
      <div class="music-actions">
        <button class="preview-btn" onclick="previewMusic('${music.url}',${music.id})">▶</button>
...
function purchaseReward(rewardId, cost) {
  const userPoints = currentUser?.rewardPoints || 0;
  if (userPoints < cost) {
    showMessage(⚠ Need ${cost - userPoints} more points, 'error');
    return;
  }
...
function openVerify(name, emailDomain) {
  if (currentUser && currentUser.college) {
    showMessage('⚠ Already connected to ' + currentUser.college, 'error');
    return;
  }
...
async function requestVerificationCode() {
  ...
  if (!email.endsWith(currentVerifyCollege.emailDomain)) {
    return showMessage('⚠ Must end with ' + currentVerifyCollege.emailDomain, 'error');
  }
  try {
    showMessage('📧 Sending code...', 'success');
