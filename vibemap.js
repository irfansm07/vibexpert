...
// ==========================================
// ENHANCED COMMUNITY CHAT FEATURES
// Add these functions to your vibemap.js file
// Replace the existing chat functions with these
// ==========================================
...
function initializeEnhancedChat() {
  console.log('✨ Initializing enhanced chat features');
  setupChatInputEnhancements();
  setupMessageActions();
  setupTypingIndicator();
  setupReactionSystem();
...
// Enhanced Chat Input with Real-time Features
function setupChatInputEnhancements() {
  const chatInput = document.getElementById('chatInput');
  if (!chatInput) return;
...
// Enhanced Message Send with Optimizations
async function sendEnhancedMessage() {
  const chatInput = document.getElementById('chatInput');
  const content = chatInput?.value.trim();
  ...
  try {
    ...
    if (tempMessage) {
      tempMessage.querySelector('.message-text').style.background = 'rgba(239, 68, 68, 0.3)';
      tempMessage.querySelector('.message-text').style.borderColor = '#ef4444';
    }
  }
}
...
function appendMessageToChat(msg) {
  ...
  messageHTML += `
    ...
    <div class="message-footer">
      ...
      <div class="message-actions">
        <button class="message-action-btn" onclick="addReaction('${messageId}', '❤️')" title="React">❤️</button>
...
function createReactionBar(messageId, reactions) {
  ...
  if (reactions && Array.isArray(reactions)) {
    reactions.forEach(r => {
      ...
      if (r.user_id && currentUser && r.user_id === currentUser.id) {
  ...
  allEmojis.forEach(emoji => {
    ...
    html += `
      <div class="reaction-pill ${selected}" onclick="toggleReaction('${messageId}', '${emoji}')">
  ...
  html += `<div class="reaction-pill" onclick="showEmojiPickerForMessage('${messageId}')" title="Add reaction">✚</div>`;
  html += '</div>';
...
function handleTypingIndicator() {
  ...
  if (now - lastTypingEmit > 2000 && socket && currentUser.college) {
    socket.emit('typing', { 
      collegeName: currentUser.college, 
      username: currentUser.username 
    });
...
function setupMessageActions() {
  document.addEventListener('click', (e) => {
    ...
    // Handle different actions based on button content/title
    const title = actionBtn.getAttribute('title');
    if (title === 'React') {
      // React action handled by onclick
    } else if (title === 'Copy') {
...
function updateConnectionStatus() {
  ...
  if (connectionStatus === 'disconnected') {
    ...
    banner.textContent = '⚠️ Disconnected - Reconnecting...';
  } else {
    if (banner) {
...
// Message Optimization - Batch Updates
...
function playMessageSound(type) {
  ...
  const audio = new Audio(sounds[type]);
  audio.volume = 0.2;
  audio.play().catch(() => {}); // Fail silently if audio doesn't play
}
...
function handlePasteEvent(e) {
  ...
  for (let item of items) {
    if (item.type.indexOf('image') !== -1) {
      ...
      // You can extend this to upload images to chat
      console.log('Image pasted:', blob);
      showMessage('📷 Image paste detected! Feature coming soon.', 'success');
    }
  }
}
...
// Add Message to UI (wrapper for appendMessageToChat)
function addMessageToUI(message) {
...
function setupEnhancedSocketListeners() {
  ...
  socket.on('message_reaction_updated', (data) => {
    // Update reaction display in real-time
    const messageEl = document.getElementById(`msg-${data.messageId}`);
    if (messageEl) {
      // Refresh reaction bar
      // This would need the full reaction data from server
    }
  });
}

// Initialize Everything When Chat Opens
document.addEventListener('DOMContentLoaded', () => {
  const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
      const chatSection = document.getElementById('chatSection');
      if (mutation.target === chatSection && 
        ...
        console.log('🎉 Enhanced chat fully initialized!');
      }
    });
  });
