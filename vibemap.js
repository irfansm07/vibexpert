...
/* ==========================================
   CHAT AUTO-SCROLL & TYPING INDICATOR FIXES
   Add these JavaScript functions to your vibemap.js file
   ========================================== */
...
/**
 * Checks if user has scrolled to near the bottom
 * @returns {boolean}
 */
...
/**
 * Modified displayChatMessage function with auto-scroll
 * Replace your existing displayChatMessage function with this
 */
function displayChatMessage(sender, text, isOwn = false) {
  const chatMessages = document.getElementById('chatMessages');
  if (!chatMessages) return;
...
let typingTimeout;
const typingIndicators = new Set();

/**
 * Shows typing indicator for a user
...
function showTypingIndicator(username) {
  ...
  typingIndicators.add(username);
  updateTypingIndicator();
}

/**
 * Hides typing indicator for a user
 * @param {string} username - Username to remove
 */
function hideTypingIndicator(username) {
  typingIndicators.delete(username);
  updateTypingIndicator();
}
...
function updateTypingIndicator() {
  ...
  if (!container) {
    ...
    if (chatBox && inputBox) {
      chatBox.insertBefore(container, inputBox);
    }
  }
  ...
  if (typingIndicators.size > 0) {
    const names = Array.from(typingIndicators);
 ...
 * Call this function when user types in chat input
 */
function handleChatInputTyping() {
  ...
  input.addEventListener('input', function() {
    if (!socket || !currentCommunity) return;
  ...
  // Also emit stop when user stops typing (on blur)
...
/**
 * Add these socket event listeners to your existing socket code
 */
function setupChatSocketListeners() {
  ...
  socket.on('chat-message', (data) => {
    const isOwn = data.user === currentUser?.name;
    displayChatMessage(data.user, data.message, isOwn);
...
/**
 * Call this function when community chat is opened
 */
function initializeChatFixes() {
  ...
  if (chatMessages) {
    chatMessages.addEventListener('scroll', function() {
      ...
      // For example, show a "scroll to bottom" button if user scrolls up
    });
  }
}
...
/**
 * Creates a "scroll to bottom" button that appears when user scrolls up
 * Call this in your chat initialization
 */
function addScrollToBottomButton() {
  ...
  button.style.cssText = `
    ...
    font-weight: 700;
    cursor: pointer;
    box-shadow: 0 8px 25px rgba(79, 116, 163, 0.4);
    z-index: 100;
    display: none;
    transition: all 0.3s ease;
  `;
  
  button.onclick = () => scrollChatToBottom(true);
...
/*
// In your existing joinCommunity function, add:

function joinCommunity(collegeName) {
  currentCommunity = collegeName;
...
// In your existing sendChatMessage function, ensure it calls displayChatMessage:

function sendChatMessage() {
  const input = document.getElementById('chatInput');
  const message = input.value.trim();
  
  if (!message || !socket || !currentCommunity) return;
  ...
  // Display own message
  displayChatMessage(currentUser.name, message, true);
