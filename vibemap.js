// VIBEXPERT - ENHANCED VERSION WITH PROFILE & COMMUNITY POST SEPARATION
...
function initializeSocket() {
  ...
  socket.on('connect', () => {
    ...
    if (currentUser?.college) socket.emit('join_community', currentUser.college);
  ...
  socket.on('new_community_post', (post) => {
    if (currentPostView === 'community') loadPosts('community');
  });
  socket.on('new_profile_post', ({ userId, post }) => {
    if (currentPostView === 'profile' && currentUser && userId === currentUser.id) loadPosts('profile');
  });
...
async function createPost() {
  ...
  if (selectedPostDestination === 'community') {
    if (!currentUser || !currentUser.communityJoined || !currentUser.college) {
      msg('⚠️ You must join a college community first to post to the community feed!', 'error');
      setTimeout(() => showPage('communities'), 2000);
      return;
    }
  }
  ...
  try {
    ...
    if(data.success) {
      const destinationName = selectedPostDestination === 'community' ? 'community feed' : 'profile';
      msg(`🎉 Posted to ${destinationName}!`, 'success');
      if (data.badges && data.badges.length > (currentUser.badges?.length || 0)) {
        currentUser.badges = data.badges;
        localStorage.setItem('user', JSON.stringify(currentUser));
      ...
      loadPosts(selectedPostDestination);
      currentPostView = selectedPostDestination;
    }
  } catch (error) {
    if (error.message.includes('community')) {
      msg('⚠️ ' + error.message, 'error');
      setTimeout(() => showPage('communities'), 2000);
    } else {
...
async function loadPosts(viewType = 'profile') {
  const feedEl = document.getElementById('postsFeed');
  if(!feedEl) return;
  currentPostView = viewType;
  const postsSection = document.getElementById('posts');
  if (postsSection) {
    let toggleHTML = `<div style="display:flex; gap:10px; margin-bottom:20px; justify-content:center;"><button onclick="loadPosts('profile')" class="feature-btn ${viewType === 'profile' ? 'active' : ''}" style="flex:1; max-width:200px;">👤 My Profile Posts</button><button onclick="loadPosts('community')" class="feature-btn ${viewType === 'community' ? 'active' : ''}" style="flex:1; max-width:200px;">🌍 Community Posts</button></div>`;
    let existingToggle = postsSection.querySelector('.post-view-toggle');
    if (existingToggle) {
      existingToggle.innerHTML = toggleHTML;
    } else {
      const toggleDiv = document.createElement('div');
      toggleDiv.className = 'post-view-toggle';
      toggleDiv.innerHTML = toggleHTML;
      postsSection.insertBefore(toggleDiv, feedEl.parentElement);
    }
  }
  try {
    feedEl.innerHTML = '<div style="text-align:center; padding:20px; color:#888;">Loading posts...</div>';
    let endpoint = '/api/posts?type=' + viewType;
    const data = await apiCall(endpoint, 'GET');
    if(!data.posts || data.posts.length === 0) {
      const emptyMessage = viewType === 'community' ? 'No community posts yet. Be the first to post to your community! 🌍' : 'No posts yet. Create your first post! 📝';
    ...
    data.posts.forEach(post => {
      ...
      html += `<div class="enhanced-post" data-post-id="${post.id}"><div class="enhanced-post-header"><div class="enhanced-user-info"><div class="enhanced-user-avatar" style="width:48px; height:48px; border-radius:50%; display:flex; align-items:center; justify-content:center; background:linear-gradient(135deg, rgba(79,116,163,0.3), rgba(141,164,211,0.3)); border:2px solid #4f74a3;">${post.users?.profile_pic ? `<img src="${post.users.profile_pic}" style="width:100%; height:100%; border-radius:50%; object-fit:cover;">` : '👤'}</div><div class="enhanced-user-details"><div class="enhanced-username">@${author}</div><div class="enhanced-post-meta"><span>${time}</span><span>•</span><span>${postedTo}</span></div></div></div>${isOwn ? `<button class="post-delete-btn" onclick="deletePost('${post.id}')">🗑️ Delete</button>` : ''}</div><div class="enhanced-post-content">${content ? `<div class="enhanced-post-text">${content}</div>` : ''}${stickers.length > 0 ? `<div class="post-stickers-container">${stickers.map(sticker => `<span class="post-sticker">${sticker.emoji || sticker}</span>`).join('')}</div>` : ''}${music ? `<div class="post-music-container"><div class="music-player"><div class="music-info"><div class="music-icon">${music.emoji || '🎵'}</div><div class="music-details"><div class="music-name">${music.name}</div><div class="music-duration">${music.artist} • ${music.duration}</div></div></div><audio controls class="post-audio-player"><source src="${music.url}" type="audio/mpeg">Your browser does not support the audio element.</audio></div></div>` : ''}${media.length > 0 ? `<div class="enhanced-post-media">${media.map(m => m.type === 'image' ? `<div class="enhanced-media-item"><img src="${m.url}" alt="Post image"></div>` : `<div class="enhanced-media-item"><video src="${m.url}" controls></video></div>`).join('')}</div>` : ''}</div><div class="enhanced-post-footer"><div class="enhanced-post-stats"><span>❤️ 0</span><span>💬 0</span><span>🔄 0</span></div><div class="enhanced-post-engagement"><button class="engagement-btn">❤️ Like</button><button class="engagement-btn">💬 Comment</button><button class="engagement-btn">🔄 Share</button></div></div></div>`;
    });
...
function showMusicSelector() {
  ...
  musicLibrary.forEach(song => {
    ...
    html += `<div class="song-item ${isSelected ? 'selected' : ''}" onclick="selectMusic(${song.id})"><div class="song-info"><div class="song-name">${song.emoji} ${song.name}</div><div class="song-duration">${song.artist} • ${song.duration}</div></div><div class="song-preview"><button class="play-btn" onclick="event.stopPropagation(); previewMusic('${song.url}')">▶️</button></div></div>`;
  });
  html += '</div>';
...
function searchStickers() {
  ...
  let allStickers = [];
  Object.values(stickerLibrary).forEach(categoryStickers => { allStickers = allStickers.concat(categoryStickers); });
...
function updateSelectedAssets() {
  ...
  selectedStickers.forEach((sticker, index) => { html += `<div class="selected-asset"><span>${sticker.emoji}</span><button onclick="removeSticker(${index})">✕</button></div>`; });
...
function showPostDestinationModal() {
  if (!currentUser || !currentUser.communityJoined || !currentUser.college) {
    msg('⚠️ Join a college community first to post to community feed!', 'error');
    setTimeout(() => showPage('communities'), 2000);
    return;
  }
...
function selectPostDestination(destination) {
  if (destination === 'community' && (!currentUser || !currentUser.communityJoined || !currentUser.college)) {
    msg('⚠️ Join a college community first!', 'error');
  ...
  setTimeout(() => {
    ...
    document.querySelectorAll('.aspect-ratio-btn').forEach(btn => {
      btn.addEventListener('click', function() {
        ...
        if (ratio === 'free') { cropper.setAspectRatio(NaN); } else { cropper.setAspectRatio(eval(ratio)); }
      });
    });
  }, 100);
}
