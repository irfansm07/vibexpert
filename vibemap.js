...
// All Features: About Us â†’ Login â†’ Main App
// ========================================
...
const rewardsData = {
  dailyTasks: [
    { id: 'post_today', title: 'Share Your Day', desc: 'Create 1 post', reward: 10, icon: 'ðŸ“', completed: false },
    { id: 'comment_5', title: 'Engage', desc: 'Comment on 5 posts', reward: 15, icon: 'ðŸ’¬', completed: false },
    { id: 'like_10', title: 'Spread Love', desc: 'Like 10 posts', reward: 5, icon: 'â¤ï¸', completed: false },
    { id: 'login_streak', title: 'Daily Login', desc: '7 days streak', reward: 50, icon: 'ðŸ”¥', completed: false }
  ],
  achievements: [
    { id: 'social', title: 'Social Butterfly', desc: '50 connections', reward: 100, icon: 'ðŸ¦‹', progress: 0, target: 50 },
    { id: 'content', title: 'Content King', desc: '100 posts', reward: 200, icon: 'ðŸ‘‘', progress: 0, target: 100 },
    { id: 'influencer', title: 'Influencer', desc: '1000 likes', reward: 500, icon: 'â­', progress: 0, target: 1000 },
    { id: 'hero', title: 'Community Hero', desc: '500 messages', reward: 150, icon: 'ðŸ¦¸', progress: 0, target: 500 }
  ],
  exclusiveRewards: [
    { id: 'theme', title: 'Premium Themes', desc: 'Exclusive themes', cost: 500, icon: 'ðŸŽ¨', category: 'cosmetic' },
    { id: 'frame', title: 'Golden Frame', desc: 'Profile border', cost: 300, icon: 'ðŸ–¼ï¸', category: 'cosmetic' },
    { id: 'badge', title: 'Custom Badge', desc: 'Your own badge', cost: 800, icon: 'ðŸ†', category: 'premium' },
    { id: 'adfree', title: 'Ad-Free 30 Days', desc: 'No distractions', cost: 1000, icon: 'ðŸš€', category: 'utility' },
    { id: 'early', title: 'Early Access', desc: 'New features first', cost: 600, icon: 'âš¡', category: 'premium' },
    { id: 'boost', title: 'Post Boost 5x', desc: 'Boost 5 posts', cost: 400, icon: 'ðŸ“¢', category: 'utility' }
  ],
  leaderboard: [
    { rank: 1, name: 'TechMaster', points: 5420, avatar: 'ðŸ‘¨â€ðŸ’»', trend: 'up' },
    { rank: 2, name: 'VibeQueen', points: 4890, avatar: 'ðŸ‘¸', trend: 'up' },
    { rank: 3, name: 'CodeNinja', points: 4250, avatar: 'ðŸ¥·', trend: 'down' },
    { rank: 4, name: 'StudyBuddy', points: 3870, avatar: 'ðŸ“š', trend: 'up' },
    { rank: 5, name: 'MusicLover', points: 3420, avatar: 'ðŸŽµ', trend: 'same' }
  ]
};

const musicLibrary = [
  { id: 1, name: "Chill Vibes", artist: "LoFi Beats", duration: "2:30", url: "https://assets.mixkit.co/music/preview/mixkit-chill-vibes-239.mp3", emoji: "ðŸŽ§" },
  ...
  { id: 3, name: "Dreamy Piano", artist: "Classical", duration: "2:45", url: "https://assets.mixkit.co/music/preview/mixkit-dreamy-piano-1171.mp3", emoji: "ðŸŽ¹" },
  { id: 4, name: "Summer Vibes", artist: "Tropical", duration: "3:30", url: "https://assets.mixkit.co/music/preview/mixkit-summer-vibes-129.mp3", emoji: "ðŸ–ï¸" },
  { id: 5, name: "Happy Day", artist: "Pop Rock", duration: "2:50", url: "https://assets.mixkit.co/music/preview/mixkit-happy-day-583.mp3", emoji: "ðŸ˜Š" },
  { id: 6, name: "Relaxing Guitar", artist: "Acoustic", duration: "3:10", url: "https://assets.mixkit.co/music/preview/mixkit-relaxing-guitar-243.mp3", emoji: "ðŸŽ¸" }
];
...
document.addEventListener('DOMContentLoaded', function() {
  console.log('ðŸš€ VibeXpert initializing...');
...
function createScrollProgressIndicator() {
  ...
  scrollProgressIndicator.innerHTML = 'ðŸ“œ Scroll to explore â€¢ <span id="scrollPercent">0%</span>';
...
function showAuthPopupAutomatic() {
  console.log('ðŸŽ‰ User reached bottom - showing auth popup');
  showAuthPopup();
  createConfetti();
}
...
function initializeMusicPlayer() {
  ...
  window.musicPlayer.addEventListener('loadedmetadata', () => console.log('ðŸŽµ Music loaded'));
  window.musicPlayer.addEventListener('error', (e) => {
    console.error('âŒ Music error:', e);
...
async function signup(e) {
  ...
  try {
    ...
    showMessage('ðŸŽ‰ Account created! Check email', 'success');
    const form = document.getElementById('signupForm');
    if (form) form.reset();
    setTimeout(() => goLogin(null), 2000);
  } catch(error) {
    showMessage('âŒ ' + error.message, 'error');
  }
}
...
async function handleForgotPassword(e) {
  ...
  try {
    showMessage('ðŸ“§ Sending code...', 'success');
...
async function verifyResetCode(e) {
  ...
  try {
    showMessage('ðŸ” Verifying...', 'success');
    await apiCall('/api/reset-password', 'POST', { email, code, newPassword });
    showMessage('âœ… Password reset! Login now', 'success');
...
async function resendResetCode() {
  ...
  try {
    showMessage('ðŸ“§ Resending...', 'success');
    await apiCall('/api/forgot-password', 'POST', { email });
...
function logout() {
  ...
  showMessage('ðŸ‘‹ Logged out', 'success');
...
async function createPost() {
  ...
  try {
    showMessage('ðŸ“¤ Creating...', 'success');
    ...
    if (selectedFiles.length > 0) {
      showMessage(`ðŸ“¤ Uploading ${selectedFiles.length} file(s)...`, 'success');
    ...
    if (data.success) {
      const msg = selectedPostDestination === 'profile' ? 'âœ… Posted to profile!' : 'âœ… Shared to community!';
      ...
      if (data.badgeUpdated && data.newBadges?.length > 0) {
        setTimeout(() => showMessage(`ðŸ† Badge: ${data.newBadges.join(', ')}`, 'success'), 6000);
      }
      resetPostForm();
      setTimeout(() => {
        loadPosts();
      ...
      showMessage('âŒ Failed', 'error');
    }
  } catch(error) {
    console.error('âŒ Post error:', error);
    if (error.message.includes('timeout')) {
      ...
      showMessage('âŒ Error: ' + error.message, 'error');
    }
  }
}
...
function renderPosts(posts) {
  ...
  posts.forEach(post => {
    ...
    const postedTo = post.posted_to === 'community' ? 'ðŸŒ Community' : 'ðŸ‘¤ Profile';
    ...
    html += `
      <div class="enhanced-post" id="post-${post.id}">
        <div class="enhanced-post-header">
          <div class="enhanced-user-info" onclick="showUserProfile('${authorId}')" style="cursor:pointer;">
            <div class="enhanced-user-avatar">
              ${post.users?.profile_pic ? `<img src="${post.users.profile_pic}" class="enhanced-user-avatar">` : 'ðŸ‘¤'}
            </div>
            <div class="enhanced-user-details">
              <div class="enhanced-username">@${author}</div>
              <div class="enhanced-post-meta"><span>${time}</span><span>â€¢</span><span>${postedTo}</span></div>
            </div>
          </div>
          ${isOwn ? `<button class="post-delete-btn" onclick="deletePost('${post.id}')">ðŸ—‘ï¸</button>` : ''}
        </div>
        <div class="enhanced-post-content">
          ...
          ${music ? `<div class="post-music-container"><div class="music-player"><div class="music-info"><div class="music-icon">${music.emoji || 'ðŸŽµ'}</div><div class="music-details"><div class="music-name">${music.name}</div><div class="music-duration">${music.artist} â€¢ ${music.duration}</div></div></div><audio controls class="post-audio-player"><source src="${music.url}" type="audio/mpeg"></audio></div></div>` : ''}
        ...
        <div class="enhanced-post-footer">
          <div class="enhanced-post-stats">
            ...
            <span id="comment-count-${post.id}">ðŸ’¬ ${commentCount}</span>
            <span id="share-count-${post.id}">ðŸ”„ ${shareCount}</span>
          </div>
          <div class="enhanced-post-engagement">
            <button class="engagement-btn ${isLiked ? 'liked' : ''}" id="like-btn-${post.id}" onclick="toggleLike('${post.id}')">${isLiked ? 'â¤ï¸ Liked' : 'â¤ï¸ Like'}</button>
            <button class="engagement-btn" onclick="openCommentModal('${post.id}')">ðŸ’¬ Comment</button>
            <button class="engagement-btn" onclick="sharePost('${post.id}', '${content.replace(/'/g, "\\'")}', '${author}')">ðŸ”„ Share</button>
          </div>
        </div>
      </div>
    `;
...
async function loadPosts() {
  ...
  try {
    ...
    if (!data.posts || data.posts.length === 0) {
      feedEl.innerHTML = '<div style="text-align:center;padding:40px;color:#888;">ðŸ“ No posts yet</div>';
      return;
    }
    feedEl.innerHTML = renderPosts(data.posts);
  } catch(error) {
    console.error('âŒ Load posts:', error);
    feedEl.innerHTML = '<div style="text-align:center;padding:40px;color:#ff6b6b;">âŒ Failed to load</div>';
  }
}
...
async function deletePost(postId) {
  ...
  try {
    ...
    showMessage('ðŸ—‘ï¸ Deleted', 'success');
    const postEl = document.getElementById(`post-${postId}`);
    if (postEl) postEl.remove();
    setTimeout(() => loadPosts(), 500);
  } catch(error) {
    showMessage('âŒ Failed: ' + error.message, 'error');
  }
}
...
async function toggleLike(postId) {
  ...
  try {
    ...
    console.error('âŒ Like:', error);
    showMessage('âŒ Failed to like', 'error');
    const likeBtn = document.querySelector(`#like-btn-${postId}`);
  ...
  modal.innerHTML = `
    <div class="modal-box" style="max-width:600px;max-height:80vh;overflow-y:auto;">
      ...
      <div style="border-top:1px solid rgba(79,116,163,0.2);padding-top:20px;">
        ...
        <button onclick="submitComment('${postId}')" style="width:100%;margin-top:10px;">ðŸ’¬ Post</button>
      </div>
    </div>
  `;
...
async function loadComments(postId) {
  ...
  try {
    ...
    if (!data.success || !data.comments || data.comments.length === 0) {
      container.innerHTML = '<div style="text-align:center;padding:20px;color:#888;">ðŸ’¬ No comments yet</div>';
      return;
    }
    let html = '';
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
            <div style="display:flex;align-items:center;gap:10px;">
              <div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#4f74a3,#8da4d3);display:flex;align-items:center;justify-content:center;font-size:18px;">
                ${comment.users?.profile_pic ? `<img src="${comment.users.profile_pic}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">` : 'ðŸ‘¤'}
              </div>
              <div>
            ...
            ${isOwn ? `<button onclick="deleteComment('${comment.id}','${postId}')" style="background:rgba(255,107,107,0.2);color:#ff6b6b;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:12px;">ðŸ—‘ï¸</button>` : ''}
          </div>
    ...
    console.error('âŒ Load comments:', error);
    container.innerHTML = '<div style="text-align:center;padding:20px;color:#ff6b6b;">âŒ Failed to load</div>';
  }
}
  ...
  try {
    ...
    if (data.success) {
      ...
      if (commentCount) {
        ...
        commentCount.textContent = `ðŸ’¬ ${currentCount + 1}`;
      }
    }
  } catch(error) {
    console.error('âŒ Comment:', error);
    showMessage('âŒ Failed to post', 'error');
  }
}
...
async function deleteComment(commentId, postId) {
  ...
  try {
    ...
    if (commentCount) {
      const currentCount = parseInt(commentCount.textContent.replace(/\D/g, '')) || 0;
      if (currentCount > 0) commentCount.textContent = `ðŸ’¬ ${currentCount - 1}`;
    }
  } catch(error) {
    console.error('âŒ Delete comment:', error);
    showMessage('âŒ Failed', 'error');
  }
}
    ...
    <div class="modal-box" style="max-width:500px;">
      ...
      <div style="background:rgba(15,25,45,0.9);border:1px solid rgba(79,116,163,0.2);border-radius:12px;padding:20px;margin:20px 0;">
        <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:15px;">
          <button onclick="shareVia('copy','${postUrl}')" class="share-option-btn"><span style="font-size:32px;">ðŸ“‹</span><span>Copy Link</span></button>
          <button onclick="shareVia('whatsapp','${postUrl}','${encodeURIComponent(shareText)}')" class="share-option-btn"><span style="font-size:32px;">ðŸ’¬</span><span>WhatsApp</span></button>
          <button onclick="shareVia('twitter','${postUrl}','${encodeURIComponent(shareText)}')" class="share-option-btn"><span style="font-size:32px;">ðŸ¦</span><span>Twitter</span></button>
          <button onclick="shareVia('native','${postUrl}','${encodeURIComponent(shareText)}')" class="share-option-btn"><span style="font-size:32px;">ðŸ“¤</span><span>More</span></button>
        </div>
      </div>
  ...
  try {
    ...
    if (postId) {
      ...
      if (shareCount) {
        ...
        shareCount.textContent = `ðŸ”„ ${currentCount + 1}`;
      }
    }
  } catch(error) {
...
function openCamera() {
  if (navigator.mediaDevices?.getUserCamera) {
    navigator.mediaDevices.getUserMedia({ video: true })
      .then(() => {
        showMessage('ðŸ“· Camera ready', 'success');
        setTimeout(() => {
          const input = document.createElement('input');
          input.type = 'file';
          input.accept = 'image/*';
      ...
      .catch(() => {
        showMessage('âš ï¸ Camera unavailable', 'error');
        openPhotoGallery();
      });
  } else {
    openPhotoGallery();
...
async function handlePhotoSelection(files) {
  ...
  showMessage('ðŸ“¸ Processing...', 'success');
  ...
  if (selectedFiles.length > 0) showMessage(`âœ… ${selectedFiles.length} photo(s) ready`, 'success');
}
      ...
      img.onload = () => {
        try {
          ...
          canvas.toBlob((blob) => {
            ...
            console.log(`ðŸ—œï¸ ${(file.size/1024).toFixed(0)}KB â†’ ${(compressedFile.size/1024).toFixed(0)}KB`);
      ...
      <div class="preview-item">
        <div class="preview-image-container">
          ...
          <div class="media-actions">
            <button class="remove-btn" onclick="removePhoto(${index})">ðŸ—‘ï¸</button>
          </div>
        </div>
      </div>
    `;
...
function removePhoto(index) {
  ...
  displayPhotoPreviews();
  showMessage('ðŸ—‘ï¸ Removed', 'success');
}
  ...
  musicLibrary.forEach(music => {
    ...
    musicItem.innerHTML = `
      ...
      <div class="music-actions">
        <button class="preview-btn" onclick="previewMusic('${music.url}',${music.id})">â–¶ï¸</button>
        <button class="select-btn ${isSelected ? 'selected' : ''}" onclick="selectMusic(${music.id})">${isSelected ? 'âœ“ Selected' : 'âœ… Select'}</button>
      </div>
    `;
...
function selectMusic(musicId) {
  ...
  closeMusicSelector();
  showMessage(`ðŸŽµ "${selectedMusic.name}" added!`, 'success');
...
function removeMusic() {
  selectedMusic = null;
  updateSelectedAssets();
  showMessage('ðŸŽµ Music removed', 'success');
}
...
function addSticker(emoji, name) {
  ...
  showMessage(`ðŸŽ¨ "${name}" added!`, 'success');
}

function removeStickers() {
  selectedStickers = [];
  updateSelectedAssets();
  showMessage('ðŸŽ¨ Stickers removed', 'success');
}
...
function updateSelectedAssets() {
  ...
  let html = '';
  if (selectedMusic) {
    html += `<div class="selected-asset"><span>ðŸŽµ ${selectedMusic.name}</span><button onclick="removeMusic()" class="remove-asset-btn">âœ•</button></div>`;
  }
  if (selectedStickers.length > 0) {
    html += `<div class="selected-asset selected-stickers"><span>ðŸŽ¨ Stickers:</span>${selectedStickers.map(s => `<span class="sticker-preview">${s.emoji}</span>`).join('')}<button onclick="removeStickers()" class="remove-asset-btn">âœ•</button></div>`;
  }
  container.innerHTML = html;
...
function selectPostDestination(destination) {
  ...
  closeModal('postDestinationModal');
  showMessage(`ðŸ“ Post to ${displayText}`, 'success');
}
...
function showPostCelebrationModal(postCount) {
  ...
  modal.innerHTML = `
    <div class="celebration-modal-content">
      ...
      <button class="celebration-button" style="background:linear-gradient(135deg,${milestone.color},${milestone.color}dd);box-shadow:0 4px 15px ${milestone.color}40;" onclick="closeCelebrationModal()">ðŸš€ Keep Posting!</button>
      ${postCount >= 10 ? `<button class="celebration-share-btn" onclick="shareAchievement(${postCount})">ðŸ“¢ Share</button>` : ''}
    </div>
  `;
  document.body.appendChild(modal);
...
function getMilestoneForPost(count) {
  const milestones = {
    1: { emoji: 'ðŸŽ‰', icon: 'â­', title: 'First Post!', message: 'Congratulations!', quote: 'Every journey begins with a step', color: '#667eea' },
    5: { emoji: 'ðŸš€', icon: 'ðŸ“ˆ', title: 'Rising Star!', message: 'Building momentum!', quote: 'Consistency is key', color: '#f093fb' },
    10: { emoji: 'â­', icon: 'ðŸŽ¨', title: 'Content Creator!', message: 'Officially a creator!', quote: 'Create content that matters', color: '#feca57' },
    25: { emoji: 'ðŸ†', icon: 'ðŸ‘‘', title: 'Champion!', message: 'Crushing it!', quote: 'Champions are made', color: '#ff6b6b' },
    50: { emoji: 'ðŸ’Ž', icon: 'âœ¨', title: 'Diamond!', message: 'Legend in making!', quote: 'Shine bright', color: '#4ecdc4' },
    100: { emoji: 'ðŸ‘‘', icon: 'âš¡', title: 'Elite!', message: 'Unstoppable!', quote: 'You inspire others', color: '#a29bfe' }
  };
  if (milestones[count]) return milestones[count];
  if (count % 10 === 0) return { emoji: 'ðŸŽŠ', icon: 'ðŸ”¥', title: `${count} Posts!`, message: 'On fire!', quote: 'Keep up the work', color: '#667eea' };
  return { emoji: 'ðŸŽ‰', icon: 'âœ¨', title: 'Post Published!', message: 'Your voice matters!', quote: 'Every post brings you closer', color: '#4f74a3' };
}
...
function shareAchievement(postCount) {
  const text = `ðŸŽ‰ I just made my ${postCount}th post on VibeXpert! Join me! ðŸš€`;
  if (navigator.share) {
    navigator.share({ title: 'VibeXpert Achievement', text, url: window.location.origin }).catch(() => {});
  } else {
    navigator.clipboard.writeText(text).then(() => showMessage('âœ… Copied!', 'success'));
  }
}
...
function loadRewardsPage() {
  ...
  let html = `
    <div style="text-align:center;margin-bottom:40px;">
      <h2 style="font-size:36px;color:#4f74a3;">ðŸŽ Rewards</h2>
    ...
    <div style="margin-bottom:50px;">
      <h3 style="color:#4f74a3;font-size:24px;margin-bottom:20px;">ðŸ“‹ Daily Tasks</h3>
        ...
        ${rewardsData.dailyTasks.map(task => `
          <div class="reward-task-card ${task.completed ? 'completed' : ''}" onclick="completeTask('${task.id}')">
            ...
            <div style="display:flex;justify-content:space-between;">
              ...
              ${task.completed ? '<span style="color:#22c55e;">âœ“ Done</span>' : '<span style="color:#888;font-size:12px;">Complete</span>'}
            </div>
          </div>
        `).join('')}
      </div>
...
function completeTask(taskId) {
  showMessage('ðŸŽ‰ Task completed! +10 points', 'success');
}

function purchaseReward(rewardId, cost) {
  const userPoints = currentUser?.rewardPoints || 0;
  if (userPoints < cost) {
    showMessage(`âš ï¸ Need ${cost - userPoints} more points`, 'error');
    return;
  }
  if (confirm(`Purchase for ${cost} points?`)) {
    showMessage('ðŸŽ Reward unlocked!', 'success');
  }
}
  ...
  if (!currentUser || !currentUser.communityJoined) {
    container.innerHTML = `
      <div class="community-guidance">
        <p>ðŸŽ“ Connect to college first!</p>
...
async function loadCommunityPosts() {
  ...
  try {
    ...
    if (!data.posts || data.posts.length === 0) {
      postsContainer.innerHTML = '<div style="text-align:center;padding:20px;color:#888;">ðŸ“¸ No posts yet</div>';
      return;
    }
    postsContainer.innerHTML = renderPosts(data.posts);
  } catch(error) {
    console.error('âŒ Community posts:', error);
    if (postsContainer) {
      postsContainer.innerHTML = '<div style="text-align:center;padding:20px;color:#ff6b6b;">âŒ Failed to load</div>';
    }
  }
}
...
function appendMessageToChat(msg) {
  ...
  messageDiv.innerHTML = `
    ...
    <div style="display:flex;gap:8px;margin-top:8px;font-size:11px;color:#888;">
      ...
      <span onclick="reactToMessage('${msg.id}','ðŸ‘')" style="cursor:pointer;">ðŸ‘</span>
      <span onclick="reactToMessage('${msg.id}','ðŸ˜‚')" style="cursor:pointer;">ðŸ˜‚</span>
      <span onclick="reactToMessage('${msg.id}','ðŸ”¥')" style="cursor:pointer;">ðŸ”¥</span>
      ${canEdit ? `<span onclick="editMessage('${msg.id}','${msg.content.replace(/'/g, "\\'")}')" style="cursor:pointer;">âœï¸</span>` : ''}
      ${isOwn ? `<span onclick="deleteMessage('${msg.id}')" style="cursor:pointer;">ðŸ—‘ï¸</span>` : ''}
    </div>
  `;
...
async function sendChatMessage() {
  ...
  try {
    ...
    showMessage('âŒ Failed to send', 'error');
  }
}
...
async function editMessage(messageId, currentContent) {
  if (editingMessageId) return showMessage('âš ï¸ Finish editing first', 'error');
...
function initializeSocket() {
  ...
  socket.on('post_commented', (data) => {
    ...
    if (commentCount) commentCount.textContent = `ðŸ’¬ ${data.commentCount}`;
  });
  socket.on('post_shared', (data) => {
    const shareCount = document.querySelector(`#share-count-${data.postId}`);
    if (shareCount) shareCount.textContent = `ðŸ”„ ${data.shareCount}`;
  });
}
...
function initializeSearchBar() {
  ...
  searchBox.addEventListener('input', (e) => {
    ...
    if (searchResults) {
      searchResults.innerHTML = '<div class="no-results">ðŸ” Searching...</div>';
...
async function performUserSearch(query) {
  ...
  try {
    ...
    console.error('âŒ Search:', error);
    searchResults.innerHTML = '<div class="no-results" style="color:#ff6b6b;">âŒ Search failed</div>';
    searchResults.style.display = 'block';
  }
}
...
function displaySearchResults(users) {
  ...
  if (!searchResults) return;
  if (users.length === 0) {
    searchResults.innerHTML = '<div class="no-results">ðŸ˜” No users found</div>';
    searchResults.style.display = 'block';
    return;
  }
  let html = '';
  users.forEach(user => {
    const avatarContent = user.profile_pic ? `<img src="${user.profile_pic}" alt="${user.username}">` : 'ðŸ‘¤';
    html += `
      <div class="search-result-item" onclick="showUserProfile('${user.id}')">
        ...
        <div class="search-result-info">
          ...
          ${user.college ? `<div class="search-result-college">ðŸŽ“ ${user.college}</div>` : ''}
        </div>
      </div>
    `;
  });
...
async function showUserProfile(userId) {
  ...
  try {
    ...
    console.error('âŒ Profile:', error);
    showMessage('âŒ Failed to load', 'error');
  }
}
        ...
        <div class="profile-header">
          ...
          <div class="profile-main">
            <div class="profile-photo-section">
              <div class="profile-photo" style="${user.profile_pic ? `background-image:url('${user.profile_pic}');background-size:cover;` : ''}">${!user.profile_pic ? 'ðŸ‘¤' : ''}</div>
              ${isOwnProfile ? '<button class="avatar-upload-btn" onclick="uploadProfilePic()">ðŸ“· Change</button>' : ''}
            ...
            <div class="profile-name-section">
              ...
              ${user.college ? `<p style="color:#888;font-size:14px;">ðŸŽ“ ${user.college}</p>` : ''}
              ${user.registration_number ? `<p style="color:#888;font-size:13px;">ðŸ“‹ ${user.registration_number}</p>` : ''}
            </div>
            ${isOwnProfile ? '<button class="profile-edit-btn" onclick="toggleEditProfile()">âœï¸ Edit</button>' : ''}
          </div>
        </div>
        <div class="profile-stats-section">
          <div class="stat-card"><div class="stat-icon">ðŸ“</div><div class="stat-value">${user.postCount || 0}</div><div class="stat-title">Posts</div></div>
          <div class="stat-card"><div class="stat-icon">ðŸ†</div><div class="stat-value">${user.badges?.length || 0}</div><div class="stat-title">Badges</div></div>
          <div class="stat-card"><div class="stat-icon">â±ï¸</div><div class="stat-value">24h</div><div class="stat-title">Active</div></div>
        </div>
      </div>
    </div>
  `;
...
async function uploadProfilePic() {
  ...
  input.onchange = async (e) => {
    ...
    if (!file) return;
    try {
      showMessage('ðŸ“¤ Uploading...', 'success');
      const compressedFile = await compressImage(file);
      const formData = new FormData();
      ...
      showMessage('âŒ Upload failed', 'error');
    }
  };
  input.click();
}
...
function toggleEditProfile() {
  showMessage('âœï¸ Edit feature coming soon!', 'success');
}
...
async function loadUserProfilePosts(userId) {
  ...
  if (!container) return;
  try {
    container.innerHTML = '<div style="text-align:center;padding:20px;color:#888;">â³ Loading posts...</div>';
    const data = await apiCall(`/api/posts/user/${userId}`, 'GET');
    if (!data.posts || data.posts.length === 0) {
      container.innerHTML = '<div style="text-align:center;padding:20px;color:#888;">ðŸ“ No posts yet</div>';
      return;
    }
    container.innerHTML = renderPosts(data.posts);
  } catch(error) {
    console.error('âŒ Load user posts:', error);
    container.innerHTML = '<div style="text-align:center;padding:20px;color:#ff6b6b;">âŒ Failed to load posts</div>';
  }
}
...
function showFeedbackModal() {
  ...
  modal.innerHTML = `
    <div class="modal-box">
      ...
      <button onclick="submitFeedback()" style="width:100%;margin-top:15px;">ðŸ“¤ Send</button>
    </div>
  `;
  document.body.appendChild(modal);
  const hamburger = document.getElementById('hamburgerMenu');
...
function toggleTheme() {
  ...
  showMessage('ðŸŽ¨ Theme changed!', 'success');
  const hamburger = document.getElementById('hamburgerMenu');
  const options = document.getElementById('optionsMenu');
  if (hamburger) hamburger.style.display = 'none';
  if (options) options.style.display = 'none';
      ...
      <div class="trending-card">
        ...
        <div class="trending-footer">
          <div class="trending-engagement">
            ...
            <div class="engagement-item">ðŸ’¬ ${item.comments}</div>
          </div>
        </div>
      </div>
    `;
...
function showFullLeaderboard() {
  showMessage('ðŸ“Š Full leaderboard coming soon!', 'success');
}
...
console.log('âœ… VibeXpert - Complete Enhanced Version Loaded');
console.log('ðŸŽ‰ Features: About Us â†’ Login â†’ Main App');
console.log('ðŸš€ All functionality integrated successfully!');
